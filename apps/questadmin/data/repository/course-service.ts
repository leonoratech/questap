...
import { SubjectRepository } from './subject-service';
...
export class CourseRepository extends BaseRepository<Course> {
    ...
    async createCourse(data: CreateCourseRequest): Promise<string> {
        // Fetch instructor name from user repository
        ...
        if (data.subjectId && !subjectName) {
            try {
                const subjectRepository = new SubjectRepository()
                const subject = await subjectRepository.getById(data.subjectId)
                subjectName = subject?.name || ''
            } catch (error) {
                console.error('Error fetching subject name:', error)
                // Continue with empty subject name rather than failing the course creation
            }
        }
    ...
    async getCoursesByLanguage(language: string): Promise<Course[]> {
        try {
            ...
            throw new Error('Failed to fetch courses by language');
        }
    }
